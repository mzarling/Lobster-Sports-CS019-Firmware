...
;	sublw	0x57				; MJZ ADDED LINE, MAXIMUM FEED RATE FOR 2-LINE MODE
	sublw	0xc8				; MJZ ADDED LINE, MAXIMUM FEED RATE FOR 2-LINE MODE
; CS019B06 end
...
;	movlw	0x57				; MJZ ADDED LINE
	movlw	0xc8				; MJZ ADDED LINE	a little more than 3.5 seconds between balls
; CS019B06 end
...
; CS019B07 start
;	movlw	0x2e			; got straight to 18% duty cycle
; CS019B06 end
	movlw	0x38			; got straight to 21% duty cycle
; CS019B07 end
	movwf	delta_s
	goto	set_pwm_value
...
;	addlw	0x3a			;  W =  + delta_c	measured 14 sec feed rate, 70 balls, 23% duty cycle, pot at 12 setting = 0.89V = 45 count
; CS019B07 start
;	addlw	0x2e
; CS019B06 end
	addlw	0x38
...
; To compensate for non-linear voltage/speed curve, the pwm duty cycle is fine tuned for each increment on the feed rate dial.
	movf	feed_a2d,w
	bcf		STATUS,C
	sublw	0xe8			; 4.55V - feed_a2d
	btfss	STATUS,C		; skip if no borrow - feed_a2d < 4.55V
	goto    feed_0_2
	bcf		STATUS,C
	movf	feed_a2d,w
	sublw	0xbf			; 3.75V - feed_a2d
	btfss	STATUS,C		; skip if no borrow - feed_a2d < 3.75V
	goto    feed_2_4
	bcf		STATUS,C
	movf	feed_a2d,w
;	sublw	0xa6			; 3.25V - feed_a2d
	sublw	0xa0			; 3.15V - feed_a2d
	btfss	STATUS,C		; skip if no borrow - feed_a2d < 3.25V
	goto    feed_4_6
	bcf		STATUS,C
	movf	feed_a2d,w
	sublw	0x7a			; 2.4V - feed_a2d
	btfss	STATUS,C		; skip if no borrow - feed_a2d < 2.4V
	goto    feed_6_8
	bcf		STATUS,C
	movf	feed_a2d,w
	sublw	0x57			; 1.7V - feed_a2d
	btfss	STATUS,C		; skip if no borrow - feed_a2d < 1.7V
	goto    feed_8_10
	bcf		STATUS,C
	movf	feed_a2d,w
	sublw	0x2e			; 0.9V - feed_a2d
	btfss	STATUS,C		; skip if no borrow - feed_a2d < 0.9V
	goto    feed_10_12
	goto	feed_12_off
feed_0_2:					; 4.55V - 5.00V pot, 65% - 100% duty cycle
	movlw	0xa6			; 65% duty cycle, 0xff - 0xa6 = 89d range
	movwf	delta_s
	movlw	0xe8			; 4.55V, 0xff - 0xe8 = 23d range
	subwf	feed_a2d,w		; amount pot setting is over this pot range minimum
	movwf	delta_c
	bcf		STATUS,C
	rlf		delta_c,f		; x2
	bcf		STATUS,C
	rlf		delta_c,w		; x4
	bcf		STATUS,C
	addwf	delta_s,f		; pot change x 4 = duty cycle change
	btfss	STATUS,C
	goto	feed_end
	movlw	0xff
	movwf	delta_s
	goto	feed_end
feed_2_4:					; 3.75V - 4.55V pot, 40% - 65% duty cycle
	movlw	0x66			; 40% duty cycle, 0xa6 - 0x66 = 64d range
	movwf	delta_s
	movlw	0xbf			; 3.75V, 0xe8 - 0xbf = 41d range
	subwf	feed_a2d,w		; amount pot setting is over this pot range minimum
	movwf	delta_c
	bcf		STATUS,C
	rrf		delta_c,w		; /2
	addwf	delta_c,f		; add half
	movf	delta_c,w
	addwf	delta_s,f		; pot change x 1.5 = duty cycle change
	goto	feed_end
;feed_4_6:					; 3.25V - 3.75V pot, 31%-40% duty cycle
feed_4_6:					; 3.15V - 3.75V pot, 32%-40% duty cycle
;	movlw	0x4f			; 31% duty cycle, 0x66 - 0x4f = 23d range
	movlw	0x51			; 32% duty cycle, 0x66 - 0x51 = 21d range
	movwf	delta_s
;	movlw	0xa6			; 3.25V, 0xbf - 0xa6 = 25d range
	movlw	0xa0			; 3.15V, 0xbf - 0xa0 = 32d range
	subwf	feed_a2d,w		; amount pot setting is over this pot range minimum
	movwf	delta_c
	bcf		STATUS,C
	rrf		delta_c,w		; /2
	...
	movlw	0x66
	subwf	delta_s,w
	btfss	STATUS,C		; if borrow, new duty cycle is less than this range maximum
	goto	feed_end		; then ok
	movlw	0x66			; else fix to range maximum
	movwf	delta_s	
	goto	feed_end
;feed_6_8:					; 2.40V - 3.25V pot, 28%-31% duty cycle
feed_6_8:					; 2.40V - 3.15V pot, 28%-32% duty cycle
;	movlw	0x47			; 28% duty cycle, 0x4f - 0x47= 8d range
;	movlw	0x47			; 28% duty cycle, 0x51 - 0x47= 10d range
	movlw	0x49			; 29% duty cycle, 0x51 - 0x49= 8d range
	movwf	delta_s
;	movlw	0x7a			; 2.40V, 0xa6 - 0x7a = 44d range
	movlw	0x7a			; 2.40V, 0xa0 - 0x7a = 38d range
	subwf	feed_a2d,w		; amount pot setting is over this pot range minimum
	movwf	delta_c
	bcf		STATUS,C
	rrf		delta_c,f		; /2
	...
	bcf		STATUS,C
	movf	delta_c,w
	subwf	delta_s,f		; - 1/32
	bcf		STATUS,C
	rrf		delta_c,w		; /64
	subwf	delta_s,f		; - 1/64
	bcf		STATUS,C
	movlw	0x51
	subwf	delta_s,w
	btfss	STATUS,C		; if borrow, new duty cycle is less than this range maximum
	goto	feed_end		; then ok
	movlw	0x51			; else fix to range maximum
	movwf	delta_s	
	goto	feed_end
;feed_8_10:					; 1.70V - 2.40V pot, 26%-28% duty cycle
feed_8_10:					; 1.70V - 2.40V pot, 26%-29% duty cycle
;	movlw	0x40			; 25% duty cycle, 0x47 - 0x40 = 7d range
	movlw	0x42			; 26% duty cycle, 0x49 - 0x42 = 7d range
	movwf	delta_s
	movlw	0x57			; 1.70V, 0x7a - 0x57 = 35d range
	subwf	feed_a2d,w		; amount pot setting is over this pot range minimum
	movwf	delta_c
	bcf		STATUS,C
	rrf		delta_c,f		; /2
	...
	btfss	STATUS,C		; if borrow, new duty cycle is less than this range maximum
	goto	feed_end		; then ok
	movlw	0x49			; else fix to range maximum
	movwf	delta_s	
	goto	feed_end
;feed_10_12:				; 0.90V - 1.70V pot, 23%-25% duty cycle
feed_10_12:					; 0.90V - 1.70V pot, 23%-26% duty cycle
;	movlw	0x3b			; 23% duty cycle, 0x40 - 0x3b = 5d range
;	movlw	0x3b			; 23% duty cycle, 0x42 - 0x3b = 7d range
; CS019B07 start
	movlw	0xff			; 50% duty cycle, 0x42 - 0x3b = 7d range
; CS019B07 end
	movwf	delta_s
	movlw	0x2e			; 0.90V, 0x57 - 0x2e = 41d range
	subwf	feed_a2d,w		; amount pot setting is over this pot range minimum
	movwf	delta_c			; 
	bcf		STATUS,C
	rrf		delta_c,f		; /2
...
;	btfss	STATUS,C		; if borrow, new duty cycle is less than this range maximum
;	goto	feed_end		; then ok
;	movlw	0x42			; else fix to range maximum
;	movwf	delta_s	
	goto	feed_end
feed_12_off:
	movlw	0x0e
